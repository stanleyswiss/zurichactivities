...
exports.formatSwissAddress = formatSwissAddress;
...
async function geocodeAddress(address) {
    try {
        ...
        const response = await fetch(`https://nominatim.openstreetmap.org/search?q=${encodedAddress}&format=json&limit=1&countrycodes=ch`, {
            headers: {
                'User-Agent': ua
            }
        });
...
function formatSwissAddress(street, postalCode, city) {
    const parts = [];
    if (street)
        parts.push(street);
    if (postalCode && city) {
...
async function getFromCache(addressKey, ttlDays) {
    try {
        const record = await db_1.db.geocodeCache.findUnique({ where: { addressKey } });
        if (!record)
            return null;
...
async function saveToCache(addressKey, result) {
    try {
        await db_1.db.geocodeCache.upsert({
            where: { addressKey },
